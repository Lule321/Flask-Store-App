...
from store.models import database, Product, Category, ProductCategory, ProductOrder, Order
import time
...
if __name__ == "__main__":
    ...
    with Redis(host=Configuration.REDIS_HOST) as redis:
        with application.app_context() as context:
            while True:
                updatedProducts = redis.lpop(Configuration.REDIS_PRODUCTS_LIST)
                if updatedProducts is not None:
                    ...
                    for updatedProduct in updatedProducts:
                        ...
                        queryResult = database.session.query(Product).join(Category, Product.categories).where(Category.name.in_(categoriesList))
                        print(queryResult)
